...
function clearRouterTempFile(config) {
  let delFileList = [];
  for (let i in config.entry) {
    let fileName = config.entry[i];
    delFileList.push(config.jsPath + config.path + `/${fileName}.jsx`);
  }
  return Promise.all(delFileList).then(() => {
