...
module.exports = function(args) {
  ...
  ssrConfig = addOptPlugins(args, ssrConfig);
  ...
  .then(() => {
    if (Array.isArray(baseConfig.entry)) {
      ...
      buildConfig = addOptPlugins(args, buildConfig);
      ...
      cssConfig = addOptPlugins(args, cssConfig);
      ...
      jsConfig = addOptPlugins(args, jsConfig);
  ...
  .then(() => {
    ...
    return spawn(`"${binGulp}"`, [
      `--gulpfile "${gulpFile}"`, `publish` ],
      { shell: true, stdio: 'inherit', env: process.env, argv0: '-m -p' }
    )
  })
  .catch(err => {
    log.error(err);
...
function addOptPlugins(args, config) {
