...
function outputFile(config, page) {
  let htmlPath = pathFn.join(process.env.PWD, 'html', config.path, pathFn.dirname(page));
  let relRoot = pathFn.relative(htmlPath, process.env.PWD) + '/';
  ...
  return new Promise((resolve, reject) => {
    return copyTemplate.call(this,
      ...
      [
        ...
        { match: /{__devDir__}/g, replace: devDir },
      ...
      return copyTemplate.call(this,
        ...
        [
          { match: /{__stylePath__}/g, replace: config.stylePath },
          { match: /{__path__}/g, replace: config.path },
      ...
      return copyTemplate.call(this,
        pathFn.join(this.baseDir, config.stylePath, config.path),
        page + '.scss'
      );
    }).catch((err) => {
      reject(err);
