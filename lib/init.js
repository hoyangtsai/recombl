...
function copyConfig(dest, ans, postcss) {
  let currentDir = /^win/.test(process.platform) ? dest.replace(/\\/g, '/') : dest;
  fs.createReadStream(path.resolve(__dirname, '../tpl/config/userConfig.js'))
    .pipe(replaceStream(/\{__currentDir__\}/g, currentDir))
...
if (config) {
  ...
  inquirer.prompt(questions).then((ans) => {
    ...
    generateTpl(dest,
      { htmlPath: path.join(dest, 'client/html'),
        jsPath: path.join(dest, 'client/container'),
        cssPath: path.join(dest, 'client/style'), path: ''},
      'index'
    );
  });
}
