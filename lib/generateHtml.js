...
function reactDOMRender(fileName) {
  ReactDOM.render = (dom, place) => {
    ...
    let distHtmlPath = path.join(process.cwd(), process.env.DEV_DIR, baseConfig.path, `${fileName}.jade`);
    fs.writeFileSync(distHtmlPath, fileHtml);
  };
  let page = path.join(process.cwd(), process.env.DEV_DIR, baseConfig.path, `${fileName}.js`);
  try {
    require(page);
  } catch (e) {
