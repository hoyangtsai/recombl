...
function reactDOMRender(fileName) {
  ReactDOM.render = (dom, place) => {
    ...
    let fileHtml = String(fs.readFileSync(baseConfig.htmlPath + baseConfig.path + `/${fileName}.html`));
    fileHtml = fileHtml.replace(reg, match => { return match + html });
    fs.writeFileSync(baseConfig.htmlPath + baseConfig.path + `/${fileName}.jade`, fileHtml);
  };
