...
const userConfig = require(path.join(process.env.PWD, 'userConfig.js'));
...
function reactDOMRender(fileName) {
  ReactDOM.render = (dom, place) => {
    ...
    let htmlPath = path.join(process.env.PWD, baseConfig.htmlPath, baseConfig.path, `${fileName}.html`);
    let fileHtml = String(fs.readFileSync(htmlPath)).replace(reg, match => { return match + html });
    let distHtmlPath = path.join(process.env.PWD, process.env.DEV_DIR, baseConfig.path, `${fileName}.jade`);
    fs.writeFileSync(distHtmlPath, fileHtml);
  };
  let page = path.join(process.env.PWD, process.env.DEV_DIR, baseConfig.path, `${fileName}.js`);
  try {
    require(page);
  } catch (e) {
