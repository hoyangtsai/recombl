...
module.exports = function(rootDir, config, fileName, cssName) {
  ...
  try {
    let fullPath = path.join(rootDir, config.htmlPath, config.path, `${fileName}.html`);
    if (fs.existsSync(fullPath)) {
      console.log(`${fullPath} 已存在`.yellow);
    } else {
      let fileHtml = htmlTpl.replace(/\{__htmlTitle__\}/g, function(match) {
        return fileName;
      });
      fileHtml = fileHtml.replace(/\{__path__\}/g, function(match) {
        return config.path;
      });
      ...
      fileHtml = fileHtml.replace(/\{__devDir__\}/g, function(match) {
        return '_tmp';
      });
      funMkdirSync(path.join(rootDir, config.htmlPath, filePath));
      fs.writeFileSync(fullPath, fileHtml);
      console.log(`${fullPath} 正常生成`.green);
    }
  } catch (e) {
    console.log(`Error fileHtml: ${e}`.red);
  }
  try {
    let fullPath = path.join(rootDir, config.jsPath, config.path, `${fileName}.js`);
    if (fs.existsSync(fullPath)) {
      console.log(`${fullPath} 已存在`.yellow);
    } else {
      let fileJs = jsTpl.replace(/\{__path__\}/g, function(match) {
        return config.path;
      });
    ...
    console.log(`Error fileJs: ${e}`.red);
  }
  try {
    let fullPath = path.join(rootDir, config.cssPath, config.path, `${fileName}.scss`);
    if (fs.existsSync(fullPath)) {
      console.log(`${fullPath} 已存在`.yellow);
    } else {
      funMkdirSync(path.join(rootDir, config.htmlPath, filePath));
    ...
    console.log(`Error fileSass: ${e}`.red);
  }
}
