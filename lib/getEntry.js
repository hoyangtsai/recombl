...
module.exports = function(baseConfig) {
  ...
  let getEntryMap = function(obj, name) {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      let webpackHotDevServer = /^win/.test(process.platform) ?
        'webpack/hot/only-dev-server' : path.join(process.env.MODULE_PATH, 'webpack/hot/only-dev-server');
      let webpackDevServerClient = /^win/.test(process.platform) ?
        'webpack-dev-server/client' : path.join(process.env.MODULE_PATH, 'webpack-dev-server/client');
