...
const DEV = argv['watch'] || argv['w'] || argv['_'].includes('dev');
  ...
  if (Array.isArray(baseConfig.fileName)) {
    ...
    for (let key in baseConfig.fileName) {
      for (let i in baseConfig.fileName[key]) {
        ...
        let module = path.resolve(projDir, 'src/container', baseConfig.samePath, `${name}.js`);
        if (DEV) {
          entry += `"dist/${name}": `;
          let webpackHotDevServer = path.resolve(moduleDir, 'webpack/hot/dev-server');
          // let webpackHotDevServer = path.resolve(moduleDir, 'webpack/hot/only-dev-server');
          let webpackDevServerClient = path.resolve(moduleDir, 'webpack-dev-server/client');
          entry += `["${webpackHotDevServer}"`;
          entry += `, "${webpackDevServerClient}?http://localhost:${PORT}"`;
          entry += `, "${module}"]`;
...
function getSameConfig(baseConfig) {
  return {
    ...
    resolveLoader: {
      root: moduleDir,
      modulesDirectories: [
        'web_loaders', 'web_modules', 'node_loaders', moduleDir
        // path.resolve(projDir, 'web_loaders'), path.resolve(projDir, 'web_modules'),
        // path.resolve(projDir, 'node_loaders'), moduleDir
      ...
      loaders:[
        {
          ...
          loaders: ['react-hot',
            'babel?presets[]=es2015,presets[]=stage-0,presets[]=react,presets[]=react-hmre']
        },
        {
          test: /\.scss$/,
...
const webpackConfig = function(baseConfig) {
  ...
  return Object.assign({
    ...
    entry: entryObj,
    output: {
      path: path.join(projDir, 'dist', baseConfig.samePath),
      publicPath: '/',
      filename: '[name].js'
    },
    module: {
...
const outHtmlConfig = function(baseConfig) {
  let entryObj = getEntry(baseConfig);
  let sameConfig = getSameConfig(baseConfig);
