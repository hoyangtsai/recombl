  ...
  if (Array.isArray(baseConfig.fileName)) {
    ...
    for (let key in baseConfig.fileName) {
      for (let i in baseConfig.fileName[key]) {
        ...
        if (DEV) {
          let hotDevServer = path.resolve(moduleDir, 'webpack/hot/dev-server');
          let hotMiddleware = path.resolve(moduleDir, 'webpack-hot-middleware/client?path=/__webpack_hmr&timeout=2000&reload=true');
          entry += `["${someEntry}"`;
          entry += `, "${hotDevServer}"`;
          entry += `, "${hotMiddleware}"]`;
        } else {
