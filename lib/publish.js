...
const webpackJsConfig = require('./webpack.config.js');
...
webpackPromise(webpackOutputHtmlConfig)
  ...
  .then((htmlEnd) => {
    ...
    if (!Array.isArray(baseConfig.entry)) {
      console.info(yellow('begin Out CSS Tempfile'));
      spawnPromise('node', [ path.join(__dirname, './generateSinglePage.js') ], { env: process.env })
        .then(code => {
          if (code === 0) {
